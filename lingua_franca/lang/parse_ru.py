...
from lingua_franca.lang.common_data_ru import _ARTICLES_RU, _NUM_STRING_RU, \
...
def _extract_number_with_text_ru(tokens, short_scale=True,
    ...
    number, tokens = \
        _extract_number_with_text_ru_helper(tokens, short_scale,
                                            ordinals, fractional_numbers)
    while tokens and tokens[0].word in _ARTICLES_RU:
        tokens.pop(0)
...
def _extract_number_with_text_ru_helper(tokens,
    ...
    """
    Helper for _extract_number_with_text_ru.
...
def _extract_decimal_with_text_ru(tokens, short_scale, ordinals):
    ...
    Notes:
        While this is a helper for extractnumber_ru, it also depends on
        extractnumber_ru, to parse out the components of the decimal.
...
def _extract_whole_number_with_text_ru(tokens, short_scale, ordinals):
    ...
    for idx, token in enumerate(tokens):
        ...
        word = token.word
        if word in _ARTICLES_RU or word in _NEGATIVES:
        ...
        if word not in string_num_scale and \
                ...
                not isFractional_ru(word, short_scale=short_scale) and \
                not look_for_fractions(word.split('/')):
            words_only = [token.word for token in number_words]
            if number_words and not all([w in _ARTICLES_RU |
                ...
                and prev_word not in _ARTICLES_RU:
            number_words = [token]
        ...
        # half cup
        if val is False:
            val = isFractional_ru(word, short_scale=short_scale)
            current_val = val

        # 2 fifths
        if not ordinals:
            next_val = isFractional_ru(next_word, short_scale=short_scale)
            if next_val:
                if not val:
                    val = 1
                val = val * next_val
...
def extract_datetime_ru(string, dateNow, default_time):
    ...
    for idx, word in enumerate(words):
        ...
        if word == "now" and not datestr:
            ...
            multiplier = None
            if is_numeric(word):
                multiplier = extractnumber_ru(word)
...
def isFractional_ru(input_str, short_scale=True):
    """
    ...
    if short_scale:
        for num in _SHORT_ORDINAL_RU:
            if num > 2:
                fracts[_SHORT_ORDINAL_RU[num]] = num
    else:
        for num in _LONG_ORDINAL_RU:
            if num > 2:
                fracts[_LONG_ORDINAL_RU[num]] = num
...
def normalize_ru(text, remove_articles):
