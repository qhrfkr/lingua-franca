...
def pronounce_number_ru(num, places=2, short_scale=True, scientific=False,
    ...
    if not ordinals:
        ...
        # will default back to normal subroutine
            ...
            print('ERROR: Exception in pronounce_number_ru: {}' + repr(e))
                    ...
                    if ordi:
                        ...
                        if i * 1000 in _SHORT_ORDINAL_RU:
                            ...
                            if n not in _SHORT_SCALE_RU:
                                ...
                                number += _SHORT_SCALE_RU[num] + "th"
                            else:
                                number = _SHORT_SCALE_RU[n] + "th"
                    else:
                        number += hundreds[i]
                res.append(number)
                ordi = False
        ...
        def _long_scale(n):
            ...
            for i, z in enumerate(_split_by(n, 1000000)):
                ...
                if i:
                    ...
                    if ordi:
                        if i * 1000000 in _LONG_ORDINAL_RU:
                            if z == 1:
                            ...
                            if n not in _LONG_SCALE_RU:
                                ...
                                number += " " + _LONG_SCALE_RU[
                                    num] + "th"
                            else:
                                number = " " + _LONG_SCALE_RU[n] + "th"
                    else:
...
def nice_time_ru(dt, speech=True, use_24hour=False, use_ampm=False):
    ...
    if use_24hour:
        ...
        if string[0] == '0':
            speak += pronounce_number_ru(int(string[0])) + " "
            speak += pronounce_number_ru(int(string[1]))
        else:
            speak = pronounce_number_ru(int(string[0:2]))
        ...
        if dt.minute == 15:
            speak = "quarter past " + pronounce_number_ru(hour)
        elif dt.minute == 30:
            speak = "half past " + pronounce_number_ru(hour)
        elif dt.minute == 45:
            next_hour = (dt.hour + 1) % 12 or 12
            speak = "quarter to " + pronounce_number_ru(next_hour)
        else:
            speak = pronounce_number_ru(hour)
